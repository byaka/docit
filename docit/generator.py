...
def obj2html(o, type, toc, prefix='', hLevel=4):
   ...
   o['type2']=type
   o['prefix']=prefix
   o['navName']=prefix+type+'_'+o.name.replace('.', '__')
   ...
   # attention
   if o.attention:
      res+='<%(h2)s id="%(navName)s___example" class="text-danger"><strong>Attention</strong> <a class="headerlink text-danger" href="#%(navName)s___attention" title="Permalink to %(name)s\' attention">¶</a> </%(h2)s>'%o
      res+='\n'.join(['<pre class="bg-danger"><code>%s</code></pre>'%s for s in o.attention])
   # example
   if o.example:
   ...
   if o.tree:
      ...
      for fSett in fSetts:
         ...
         for k2 in o.tree.methods[fSett.prefix+'Order']:
            ...
            resTree+=obj2html(o2, 'method_'+fSett.prefix, tocTree, prefix=prefix+fSett.prefix+'_method_')
         if resTree:
...
def module2html(o, toc, template, prefix='', hLevel=1, root=False):
   o['prefix']=prefix
   o['navName']=prefix+'module_'+o.name.replace('.', '__')
   o['h1']='h'+str(hLevel)
   o['h2']='h'+str(hLevel+1)
   o['h3']='h'+str(hLevel+2)
   o['h4']='h'+str(hLevel+3)
   ...
   resTree=''
   for k2, o2 in o.tree.classes.items():
      resTree+=obj2html(o2, 'class', tocTree, hLevel=hLevel+2, prefix=o.navName+'_')
   if resTree:
      tocCurrent.add(tocTree)
   ...
   for fSett in fSetts:
      ...
      for k2 in o.tree.methods[fSett.prefix+'Order']:
         o2=o.tree.methods[fSett.prefix][k2]
         resTree+=obj2html(o2, 'function_'+fSett.prefix, tocTree, hLevel=hLevel+2, prefix=o.navName+'_'+fSett.prefix+'_function_')
      if resTree:
         tocCurrent.add(tocTree)
